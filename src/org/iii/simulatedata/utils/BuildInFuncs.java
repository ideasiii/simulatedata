...
public class BuildInFuncs
{
	...
	{
		return (Data.word[random.nextInt(Data.word.length - 1)]
				+ Data.word[random.nextInt(Data.word.length - 1)]);
	}
	...
	public static String strEmail()
	{
		...
		while (nSize < nLen)
		{
			salt.append(allowedChars
					.charAt(random.nextInt(allowedChars.length() - 1)));
			++nSize;
		}

		String saltStr = salt.toString();
		return (saltStr + "@"
				+ Data.email[random.nextInt(Data.email.length - 1)]);
	}
	...
	public static String strCompanyPhone()
	{
		...
		{
			salt.append(
					strNumber.charAt(random.nextInt(strNumber.length() - 1)));
			++nSize;
		}
	...
	public static String strStreet()
	{
		...
		try
		{
			...
			if (rs.next())
			{
				strRecord = rs.getString("city") + rs.getString("country")
						+ rs.getString("road");
			}
			con.close();
	...
	public static String strUUID(Integer nType)
	{
		...
		switch (nType)
		{
			case 0:
				...
				break;
			case 1: // 去掉 "-"
				strResult = s.substring(0, 8) + s.substring(9, 13)
